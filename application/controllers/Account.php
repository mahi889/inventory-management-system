...
public function inventory()
{
	...
	public function create()
	{
		...
		$this->form_validation->set_rules('father_name','Father Name','xss_clean|trim|alpha_numeric');
		$this->form_validation->set_rules('lname','Last Name','xss_clean|trim|alpha_numeric');
		$this->form_validation->set_rules('place','Place','xss_clean|trim');

		$this->form_validation->set_rules('email','Email','required|xss_clean|trim|callback_check_email');
		$this->form_validation->set_rules('contact','Contact No.','xss_clean|trim|max_length[13]|numeric');
		$this->form_validation->set_rules('address','Address','xss_clean|trim|max_length[255]');
		$this->form_validation->set_rules('position_id','Role','xss_clean|trim|max_length[1]');
			...
			$data["position"] = $this->accounts->fetch_position();
	  		...
	  		$this->load->view('CP/my_profile',$data);
				...
				$data = array('F_name' => $this->input->post('fname'), 
						  ...
						  'position_id' => $this->input->post('position_id') ? $this->input->post('position_id') : 3,
