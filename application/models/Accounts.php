...
class Accounts extends CI_Model {
	...
	public function registered($position_id = 3)
	{
		...
		$data = array('F_name' => $this->input->post('fname'), 
					  ...
					  'status' => 'In-active',
					  'position_id' => $this->input->post('position_id') ? $this->input->post('position_id') : $position_id,
					  'date_applied' => $reg
								);
	...
	function load_users($role)
	{
		
		$this ->db->select('*,account.id as empid,position.id as posid');
	   $this ->db->from('account');
	   $this->db->join('position','position.id=account.position_id','left');
	   ($role) ? $this->db->where('position.id',$role) : '';
	   $query = $this->db->get();
	   if ($query->num_rows() > 0) {
		foreach ($query->result() as $row) {
		$data[] = $row;
